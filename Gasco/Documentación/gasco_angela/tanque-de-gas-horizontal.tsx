...
const TanqueGasHorizontal = () => {
  const [gasLevel, setGasLevel] = useState(50);
  
  // Función para cambiar el nivel de gas
  const changeGasLevel = (newLevel) => {
    ...
    <div className="flex flex-col items-center p-6 bg-gray-100 rounded-lg shadow-md w-full max-w-2xl">
      ...
      <div className="relative w-full h-96 mb-4">
        ...
        <div className="absolute top-1/4 left-0 right-0 h-48 bg-gray-300 border-2 border-gray-500 rounded-full overflow-hidden">
          ...
          {/* Flotador */}
          <div className="absolute left-1/2 w-8 h-1 bg-gray-700 -ml-4"
               style={{ bottom: `${gasLevel}%` }}>
        ...
        <div className="absolute bottom-30 right-0 text-black font-bold">
          Línea de servicio
        </div>
        
        {/* Línea de llenado */}
        <div className="absolute top-24 left-0 w-16 h-2 bg-yellow-500"></div>
        <div className="absolute top-16 left-0 text-black font-bold">
          Línea de llenado
        </div>
        
        {/* Tapa protectora */}
        <div className="absolute top-8 left-1/2 -ml-20 w-40 h-24 bg-blue-500 rounded-t-full"></div>
        <div className="absolute top-2 left-1/2 -ml-16 text-black font-bold">
          Tapa protectora
        </div>
        ...
        <div className="absolute top-2 right-24 text-black font-bold">
          Válvula de servicio
        </div>
        
        {/* Regulador */}
        <div className="absolute top-12 right-12 w-8 h-12 bg-red-500"></div>
        <div className="absolute top-2 right-6 text-black font-bold">
          Regulador
        </div>
        ...
        <div className="absolute top-2 left-1/2 ml-6 text-black font-bold">
          Medidor de porcentaje
        </div>
        
        {/* Cañuelas */}
        <div className="absolute top-20 left-1/2 w-12 h-4 bg-gray-700 rounded-full"></div>
        <div className="absolute top-24 left-1/2 ml-12 text-black font-bold">
          Cañuela
        </div>
      </div>
      ...
      <div className="flex flex-col w-full gap-4">
        <div className="flex items-center justify-between">
          <button 
            ...
            onClick={() => changeGasLevel(gasLevel - 10)}
          >
            -10%
          </button>
...
export default TanqueGasHorizontal;
